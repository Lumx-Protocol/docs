...
```javascript JavaScript
axios
  .post("https://protocol-sandbox.lumx.io/v1/contracts/<contract_id_you_received_on_step_2>/item-types", {
    description: "My first item type",
    name: "First item type",
    supply: 100,
  })
...
```javascript JavaScript
axios
  .post("https://protocol-sandbox.lumx.io/v1/contracts/<contract_id_you_received_on_step_2>/deploy")
  .then((response) => {
    // Handle Success
    console.log(response);
  });
